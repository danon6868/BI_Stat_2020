...
# В данных наблюдался эффект полной коллинеарности. При этом функция vif не будет работать без дадания дополнительного параметра
# В этом случае можно помостеть, какая переменная вносит этот эффект и удалить ее из модели. В этом случае это  ARC_N 
# Но даже так VIF большинства переменных очень высок.
# Можно сразу сделать вывод, что такая модель является плохим решением, если только нужно что-то прикинуть
# Нарушаются некоторые условия применимости + большая мультиколлинеарность, от которой можно, конечно избавиться, убрав их
